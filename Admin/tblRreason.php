...
$query=mysqli_query($con,"select * from tbl_disable_res");
...
if(isset($_GET['btnadd']))
{
	if($query)
	{
		?>
			...
			?>
			<tr>
			<td><?php echo $r['vchRRea_id'];?></td>
			<td><?php echo $r['vchRRea_name'];?></td>
			<td></td>
			</tr>
			<?php } ?>
			<tr>
			...
			<td><input type="submit" class="lg" name="btnadrea" title="Save"/>&nbsp;<a href="tblRreason.php"><img src="../icons/can.png"  class = "ig" title="Cancel" name="cancel"></td></td>
			</tr>
			</table>
		<?php
	}
...
else
{
	if($query)
	{
		?>
			...
			<td><?php echo $r['vchRRea_id'];?></td>
			<td><?php echo $r['vchRRea_name'];?></td>
			<td class="tb2"><a href="tblRreason.php?reaupdt=<?php echo $r['vchRRea_id'];?>"><img src="../icons/edit.png"  class = "ig" title="Update" name="update" ></a>
				<a href="tblRreason.php?reaupdt1=<?php echo $r['vchRRea_id'];?>" onClick="return confirm('Are u sure you want to delete?');"><img src="../icons/remove.png"  class = "ig" title="Delete" name="delete" ></a>
			</tr>
			<?php } ?>
			</table>
		<?php
...
if(isset($_POST['btnadrea']))
{
	...
	if($status==0)
	{
$qdept=mysqli_query($con,"select vchRRea_id from tbl_disable_res where vchRRea_id='$_POST[txtridn]'");
...
{
		$q=mysqli_query($con,"insert into tbl_disable_res values('$_POST[txtridn]','$_POST[txtrnmn]')");
		if($q)
		{
			//echo "inserted";
			echo "<script type='text/javascript'>";
			echo "alert('Leave Reason is created successfully....')";
			echo "</script>";
			echo "<script>window.location.href='tblRreason.php';</script>";
		}
		else
		{
...
{
	//echo "dsjfhsuihjasdh";
	$s=mysqli_query($con,"select * from tbl_disable_res where vchRRea_id='$_GET[reaupdt]'");
	...
	if($s)
	{
		while($r1=mysqli_fetch_array($s))
		{
			...
			?>
			<input type="text" class="textbox" name="txtrid1" value="<?php echo $r1['vchRRea_id'];?>" readonly><br>
			<input type="text" class="textbox" style="margin:12px 0 12px 0;" name="txtrnm1" value="<?php echo $r1['vchRRea_name'];?>"><br>
...
if(isset($_POST['btnreaupdt']))
{
	...
	if($status==0)
	{
		$updt=mysqli_query($con,"update tbl_disable_res set vchRRea_id='$_POST[txtrid1]',vchRRea_name='$_POST[txtrnm1]' where vchRRea_id='$_GET[reaupdt]'");
		
		if($updt)
		{
			echo "<script type='text/javascript'>";
			echo "alert('Leave Reason is updated successfully....')";
			echo "</script>";
			echo "<script>window.location.href='tblRreason.php';</script>";
		}
	}
...
{
	$qry1="select * from tbl_sow where vchRRea_id='$_GET[reaupdt1]' ";
	$res=mysqli_query($con,$qry1);
	
	if(mysqli_fetch_array($res)==0)
	{
		
		$del=mysqli_query($con,"delete from tbl_disable_res where vchRRea_id='$_GET[reaupdt1]' ");
		
		if($del)
		{
			echo "<script type='text/javascript'>";
			echo "alert('Leave Reason is deleted successfully....')";
			echo "</script>";
			echo "<script>window.location.href='tblRreason.php';</script>";
		}
	}
...
{
	header('location:tblRreason.php');
}
echo "</center>";
